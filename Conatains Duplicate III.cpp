...
public:
	...
	/* @note	维护一个multiset，里面存放的是连续k个元素的值。因为set/map等关联容器的本质是BST，那么只需要维护住一个k元素大小的multiset即可，
				BST的查找与增加、删除操作的时间复杂度为O(logn)。算法的思想是从头开始遍历数组，维持住multiset的k大小，然后比较比当前元素相差t
				的元素是否在multiset中存在，如果存在则返回true，否则继续向后找。最后退出循环返回false。时间复杂度为O(nlogk)，空间复杂度为O(k)*/
