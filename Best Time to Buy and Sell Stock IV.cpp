...
public:
	...
	///@param	prices	每一天的股价
	///@return	返回最大利润
	/* @note	动态规划。当k < prices.size()时，设dp_local[i][j]表示到第i天至多j次交易且在第i天进行最后一次交易的的最大利润，dp_global[i][j]表示在第i天至多进行j次交易的最大利润。
				递推关系式为dp_local[i][j] = max(dp_local[i-1][j]+diff, dp_global[i-1][j-1]+max(diff, 0))，其中diff = prices[i] - prices[i-1]。如果前i-1天进行了j次交易，且第i-1
				天完成了最后一次交易，那么为了保证最后一天i也有交易，那么必须要算上diff，但这也不会增加总的交易次数，因为在第i-1天卖出，然后在第i-1天买入，再在第i天卖出，相当于在第i天
				卖出，总的交易次数并没有改变。如果在前i-1天完成了j-1次交易，那么如果diff > 0，那么就选择算上最后一天的收益，如果diff < 0，为了能够满足在最后一天有交易的要求，则选择在
				当天买入，当天卖出。
				递推关系式dp_global[i][j] = max(dp_global[i-1][j], dp_local[i][j])，如果最后一次交易在第i天，那么就是dp_local[i][j]，如果最后一次交易不在第i天，那么就是dp_global[i-1][j]。
