...
public:
	...
	//			最顶层选到了最后一个元素。
	//			计算出所有的排列后再返回其中的第k个。时间复杂度为O(n!)，空间复杂度为O(n^2)。OJ报TLE.
	string getPermutation(int n, int k) 
	{
		...
		getPermutate(nums, ivec, rslt);	//计算出所有可能的排列

		string str;
		if (k > rslt.size() || k < 1) return str;
			
		vector<int> k_prmt = rslt[k-1];
		for (int i = 0; i != k_prmt.size(); i++)
		{
			str += k_prmt[i]+'0';
		}
		return str;
...
private:
	...
	///@param	rslt	保存有全排列的数组
	///@return	void
	///@note	我的想法是每次拿掉一个元素，将剩余的元素组成一个新数组，并且记录下当前拿掉的是第几个元素；当新数组为空时需要得到一个可能的组合；
	//			此时返回上一级递归，再拿掉下一个元素。依次类推。
	void getPermutate(vector<int> nums, vector<int>& residue, vector<vector<int>>& rslt)
	{
		...
		while (indx != nums.size())	//	如果当前元素不是数组中的最后一个元素，则进入循环
		{
			...
			getPermutate(tmp, residue, rslt);		//	递归
