...
class Solution {
public:
    ///@brief   计算给定字符串的最长回文子串
    ///@param   s   字符串s
    ///@return  返回s含有的最长子串
    ///@note    Manacher's algorithm.
    /*          1. 在每个字符之间插入字符串中从未出现的字符，比如'#'，这样可以将原字符串的奇偶性全部转换为奇数长度的字符串t。
                   然后在首尾再添加异于之前插入的字符的字符，比如'$'和'^'。由于字符串末尾自带'\0'，所以末尾字符不必再额外添加。
                2. 新建一个数组p，p[i]表示以t[i]为中心的回文字符串的半径。如果只有t[i]本身回文，设半径p[i] = 1。
                3. 设mx为当前所有回文字符串能够触及的最右端边界，id为mx为右边界的回文子串的中心。则i必在id的右侧，因为如果i在id的左
                   侧或者id的位置，那么不可能获取以id为中心，mx为右边界的回文子串。
                4. 剩下需要判断i与mx的关系，分两种情况：
                    4.1 i < mx;
                        4.1.1 p[i] < mx - i，即如果以i为中心的回文子串半径小于mx - i，那么在以id为中心的另一侧，即在id的左侧
                              必定存在着一个回文子串，其以t[j]为中心，p[j]为半径，其中j = id - (i - id) = 2*id - i，使得p[i] = p[j].
                        4.1.2 p[i] >= mx - i，即如果以i为中心的回文子串的半径大于等于mx - i，那么p[i]至少等于mx - i。因为超出
                              右边界mx的部分是未知的，需要逐一比对才能确认。
                    4.2 i >= mx; 因为i已经超出或等于右边界，需要逐一比对才可以确认其最大值，那么至少应该等于1.
                    综合上面两种情况，p[i] = i < mx ? min(p[2*id - i], mx - i) : 1;
                5. 在每次遍历t[i]的时候，需要及时更新最大回文子串的半径resLen和最大回文子串的中心位置resCenter。
                6. 最终在输出最长回文子串时，应该从初始的字符串s开始剪裁（不含有额外添加的字符'#', '$'等），因为resLen和resCenter保存的是
