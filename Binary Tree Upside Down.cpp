...
public:
	...
	///@return	返回新二叉树的根节点
	///@note	1. 递归；
	//			2. 新建三个临时变量，cur_left表示当前节点root的左孩子，cur_right表示当前节点root的右孩子，new_root表示新根节点，
	//			   通过调用递归函数初始化；
	//			3. 函数结束递归的条件是当前节点为空或者当前节点的左孩子为空；
	//			4. 在递归函数跳出后，cur_left是新的子树节点，将其左孩子设置为cur_right，将其右孩子设置为当前节点；
	//			5. 将当前节点的左右孩子置为空；
	//			6. 最后返回新的根节点new_root即可。
